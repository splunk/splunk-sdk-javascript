...
(function() {
    ...
    * in the constructor, defaults to `process.stdout`.
    *
    ...
    */
    EventWriter.prototype.writeEvent = function(event) {        
        ...
        try {
            event._writeTo(that._out);
        }
        catch (e) {
            if (e.message === "Events must have at least the data field set to be written to XML.") {
                Logger.warn("", e.message, that._err);
                throw e;
            }
            Logger.error("", e.message, that._err);
            throw e;
        }
    };
    ...
    * output stream specified in the constructor, defaults to
    * `process.stdout`.
    *
