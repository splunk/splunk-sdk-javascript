...
(function() {
    ...
    // set.
    if (process.env.LOG_LEVEL && levels.hasOwnProperty(process.env.LOG_LEVEL)) {
        // $LOG_LEVEL is a recognized string specifying a level
        process.env.LOG_LEVEL = levels[process.env.LOG_LEVEL];
    } else if (process.env.LOG_LEVEL && !isNaN(parseInt(process.env.LOG_LEVEL)) 
               && utils.isValueIn(parseInt(process.env.LOG_LEVEL), levels)) {
        ; // $LOG_LEVEL is already an acceptable
    } else {
        // Anything else, default to ERROR
