...
(function() {
    ...
    var setLevel = function(level) {    
        if (utils.isString(level) && levels.hasOwnProperty(level)) {
            process.env.LOG_LEVEL = levels[level];
                   ...
                   utils.isValueIn(parseInt(level, 10), levels)) {
    ...
    if (typeof(console) !== "undefined") {
        ...
        if (console.log) { _log = log_as("log"); };
        if (console.error) { _error = log_as("error"); };
        if (console.warn) { _warn = log_as("warn"); };
        if (console.info) { _info = log_as("info"); };
    }
