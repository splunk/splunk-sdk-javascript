...
(function() {
    ...
    root.DataModel = root.Entity.extend({
         ...
         * @constructor
         */
        init: function(name, service, path) {
            this.name = name;
            ...
            this._objects = {};
            this._description = this._accelerationEnabled = this._displayName = this._earliestAcceleratedTime = this._accelerationCronSchedule = undefined;
        ...
        isAccelerated: function() {
            return this._accelerationEnabled;
        },
    ...
    root.DataModels = root.Collection.extend({
        ...
        create: function(name, params, callback) {
            ...
            return this.post("", {name: name, description: JSON.stringify(params)}, function(err, response) {
                if (err) {
                    ...
                    var dataModel = new root.DataModel(name, that.service, that.path());
         ...
         */
         instantiateEntity: function(props) {
            var entityNamespace = utils.namespaceFromProperties(props);
            var dm = new root.DataModel(props.name, this.service, this.path());

            dm._properties._accelerationEnabled = JSON.parse(props.content[dm._ACCELERATION_LABEL]).enabled;
            // TODO: handle the case when acceleration is enabled, extracting the other fields
            dm._properties.displayName = props.content[dm._DISPLAY_NAME_LABEL];

            var rawJSON = JSON.parse(props.content[dm._RAW_JSON_LABEL]);
            dm._properties.description = rawJSON[dm._DESCRIPTION_LABEL];

            /**
             * Takes an array of data model objects
             * Returns an array of cleaned up data
             * model objects.
             */
            function parseObjects(objs) {
                for (var i = 0; i < objs.length; i++) {
                    ...
                    for (var s = 0; s < calculations.length; s++) {
                        ...
                        //console.log(calculation.outputFields);
            ...
            // TODO: get other info from the rawJSON like description
            dm._properties._objects = parseObjects(rawJSON.objects);
            return dm;
         }
    });
