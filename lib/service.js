...
(function() {
    ...
    root.Endpoint = Class.extend({
        ...
        get_v2: function(relpath, params) {
            var url = this.qualifiedPath;
    ...
    root.Collection = root.Resource.extend({
        ...
        fetch_v2: async function(options) {
            options = options || {};
            if (!options.count) {
                options.count = 0;
            }
            ...
            var that = this;
            var response = await that.get_v2("", options);
            // return response;
    ...
    root.Configurations = root.Collection.extend({
        ...
        doesFileExist: async function(filename) {
            try {
                var that = this;

                // 1. Fetch files list
                var response = await this.get_v2("",  {__conf: filename});

                // 2. Filter the files
                var files = response?.body?.entry?.filter(f => f.name === filename);
                
                // 2. Check if the file exists
                if(files && files.length == 0) {
                    return { code:"FILE_NOT_FOUND", message: `File with the name '${filename}' does not exist.`};
                }
                ...
                var fetchedFile = await configurationFile.fetch_v2();

                return {file: fetchedFile};
            }
            catch(error) {
                return { errorCode:"SYSTEM_ERROR", errorMessage: error};
            }
        },

        doesStanzaExist: async function(file, stanza) {
            try {
                ...
                if(!fetchedStanza) {
                    return { code:"STANZA_NOT_FOUND", message: `Stanza with the name '${stanza}' does not exist.`};
                }
                else {
                    return {stanza: fetchedStanza};
                }
            }
            catch(error) {
                return { errorCode:"SYSTEM_ERROR", errorMessage: error};
        ...
        create_v2: async function (configs, svc, filename, stanza, keyValueMap, callback) {
            callback = callback || function() {};
            var that = this;

            // 1. Check if the file exists
            var doesFileExistResponse = await this.doesFileExist(filename);
            var configFile = doesFileExistResponse.file;

            // 2. If the file doesn't exist, create a new file
            if(!configFile) {
                console.log(doesFileExistResponse);
                console.log("Creating a new configuration file...");

                that.create( { __conf: filename }, 
                    (err, newFile) => console.log(`Configuration File created: ${filename}`) );

                configFile = new root.ConfigurationFile( that.service, filename );
                configFile = await configFile.fetch_v2();
            }

            // 2. Check if the stanza exists
            var doesStanzaExistResponse = await this.doesStanzaExist(configFile, stanza);
            var configStanza = doesStanzaExistResponse.stanza;

            // 3. If the stanza doesn't exist, create a new stanza with given keyValueMap
            if(!configStanza) {
                console.log(doesStanzaExistResponse);
                console.log("Creating a new configuration stanza...");

                configFile.create(stanza, keyValueMap, function (err, newStanza) {
                    console.log(`Configuration Stanza created: ${stanza}`);
                    callback();
                });
            }
            // 4. If the stanza exists, update it with the keyValueMap
            else {
                configStanza.update(keyValueMap, (err, updatedStanza) => {
                    console.log(`Configuration Stanza updated: ${stanza}`);
                    callback();
                });
            }
        },
