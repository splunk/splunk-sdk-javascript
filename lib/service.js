...
(function() {
    ...
    root.Collection = root.Resource.extend({
        ...
        fetchAsync: async function(options, response_timeout) {
            ...
            var that = this;
            var response = await that.get("", options, response_timeout, true);
    ...
    root.Configurations = root.Collection.extend({
        ...
        getConfFile: async function(filename, response_timeout) {
            ...
            // 1. Fetch files list
            var response = await this.get("",  {__conf: filename}, response_timeout, true);
            ...
            var fetchedFile = await configurationFile.fetchAsync(response_timeout);
