...
(function() {
    ...
    module.exports = root = Service = BaseService.extend({
        ...
        /** TODO: docs
         * Gets the `DataModels` collection...
         *
    ...
    root.DataModel = root.Entity.extend({
        ...
        init: function(service, name, props) {
            ...
            // TODO: refactor to use rawJSON more than this._properties
            // If the acceleration property is present, parse it as JSON
            if (this._properties[this._ACCELERATION_LABEL]) {
                this._properties.acceleration = JSON.parse(this._properties.acceleration);
                if (this._properties.acceleration.hasOwnProperty("enabled")) {
                    this._properties.accelerationEnabled = this._properties.acceleration.enabled;
                    // If the acceleration enabled is stored as a number, convert to boolean.
                    if (utils.isNumber(this._properties.acceleration.enabled)) {
                        this._properties.accelerationEnabled = this._properties.accelerationEnabled !== 0;
                    }
                }
                if (this._properties.acceleration.hasOwnProperty("earliest_time")) {
                    this._properties.earliestTime = this._properties.acceleration.earliest_time;
                }
            ...
            for (var i = 0; i < objs.length; i++) {
                // Split the lineage into an array
                objs[i]._lineage = objs[i].lineage.split(".");
                objs[i].lineage = function() {
                    return this._lineage;
                };

                // Convert fields to a dict with names of fields as keys
                var fields = objs[i].fields;
                objs[i]._fields = {};
                for (var f = 0; f < fields.length; f++) {
                    var field = fields[f];

                    // Split the owner lineage
                    field.ownerLineage = field.owner.split(".");
                    delete field.owner;

                    // Rename some of the keys
                    field.name = field.fieldName;
                    delete field.fieldName;
                    field.multivalued = field.multivalue;
                    delete field.multivalue;

                    field.ownerName = field.ownerLineage[field.ownerLineage.length - 1];

                    // TODO: some util functions?

                    objs[i]._fields[field.name] = field;
                }
                delete objs[i].fields;

                // TODO: test this convenience function; might be broken because of this reference
                objs[i].fields = function() {
                    return this._fields;
                };

                // TODO: test this convenience function; might be broken because of this reference
                objs[i].fieldNames = function() {
                    return Object.keys(this._fields);
                };

                // TODO: test this convenience function; might be broken because of this reference
                objs[i].fieldByName = function(name) {
                    return this._fields[name];
                };

                // TODO: test this convenience function; might be broken because of this reference
                // This function will return this._fields, AND fields for each calculation in this._calculations
                objs[i].allFields = function() {
                    var ret = this._fields;
                    for (var c = 0; c < this.calculations().length; c++) {
                        var calc = this.calculations()[c];
                        for (var f = 0; f < calc._outputFields.length; f++) {
                            var calculatedField = calc._outputFields[f];
                            ret[calculatedField.name] = calculatedField; // TODO: the same things to fields as done above
                        }
                    }
                    return ret;
                };

                objs[i].allFieldNames = function() {
                    return Object.keys(this.allFields());
                };


                // Process constraints
                var constraints = objs[i].constraints;
                objs[i]._constraints = [];
                for (var c = 0; c < constraints.length; c++) {
                    var constraint = constraints[c];

                    // Split the owner lineage, TODO: check if the Java SDK is doing this
                    constraint.ownerLineage = constraint.owner.split(".");
                    delete constraint.owner;

                    // Rename the search key
                    constraint.query = constraint.search;
                    delete constraint.search;

                    objs[i]._constraints.push(constraint);
                }

                // TODO: test this convenience function; might be broken because of this reference
                objs[i].constraints = function() { 
                    return this._constraints;
                };

                // Process calculations
                var calculations = objs[i].calculations;
                objs[i]._calculations = [];
                for (var s = 0; s < calculations.length; s++) {
                    var calculation = calculations[s];

                    // Rename keys
                    calculation.type = calculation.calculationType;
                    delete calculation.calculationType;
                    calculation.calculationId = calculation.calculationID;
                    delete calculation.calculationID;

                    calculation._outputFields = [];
                    var outputFields = calculation.outputFields;
                    for (var of = 0; of < outputFields.length; of++) {
                        var outputField = outputFields[of];

                        // Split the owner lineage
                        outputField.ownerLineage = outputField.owner.split(".");
                        delete outputField.owner;

                        // Rename keys
                        outputField.name = outputField.fieldName;
                        delete outputField.fieldName;
                        outputField.multivalued = outputField.multivalue;
                        delete outputField.multivalue;

                       calculation._outputFields.push(outputField);
                    }

                    // TODO: test this convenience function; might be broken because of this reference
                    calculation.outputFields = function() {
                        return this.outputFields;
                    };

                    objs[i]._calculations.push(calculation);
                }

                // TODO: test this convenience function; might be broken because of this reference
                objs[i].calculations = function() { 
                    return this._calculations;
                };

                if (objs[i].hasOwnProperty("groupByFields")) {
                    // TODO: handle this property
                }
                if (objs[i].hasOwnProperty("objectsToGroup")) {
                    // TODO: handle this property
                }

                // Gets the data model that contains this object
                var that = this;
                objs[i].dataModel = function() {
                    return that;
                };
                //var curObj = objs[i];
                objs[i].parent = function() {
                    return that.objectByName(this.parentName) || null;
                };
            }
            this._objects = objs;
        ...
        objects: function() {
            //var that = this;
            // TODO: clean this up without the util function, and/or move to init
            // function parseObjects(objs) {
            //     for (var i = 0; i < objs.length; i++) {
            //         // Split the lineage into an array
            //         objs[i].lineage = objs[i].lineage.split(".");
            ...
            //             objs[i]._fields.push(field);
            //         }

            //         // TODO: test this convenience function
            //         objs[i].fields = function() {
            //             return this._fields;
            //         };
            ...
            // if (!JSON.parse(this._properties[this._RAW_JSON_LABEL]).objects) {
            //     return [];
            // }
            // return parseObjects(JSON.parse(this._properties[this._RAW_JSON_LABEL]).objects);
            //return this._properties._objects;
    ...
    root.DataModels = root.Collection.extend({
        ...
        init: function(service, path, namespace) {
            ...
            this.concise = "0"; // TODO: check this
            this.create = utils.bind(this, this.create);
        },
        ...
        create: function(name, params, callback) {
            ...
            params = params || {};
            params.modelName = name; // TODO: do I need this?
            callback = callback || function(){};
