...
(function() {
    ...
    module.exports = root = Service = BaseService.extend({
        ...
        currentUser: function(response_timeout) {
            var that = this;
            var req = this.get(Paths.currentUser, {}, response_timeout).then((response)=>{
                var username = response.data.entry[0].content.username;
                var user = new root.User(that, username);
                user.fetch({}, response_timeout).then((res) => {
                    // TODO: verify the working of method
                    return res;
                })
            });
    ...
    root.Entity = root.Resource.extend({
        ...
        update: function(props, response_timeout) {
            ...
            var that = this;
            var req = this.post("", props, response_timeout).then((response)=>{
                if (!that.fetchOnUpdate){
                    that._load(response.data.entry);
                    return that;
                }
                else{
                    that.fetch({}, response_timeout).then((res)=>{
                        // TODO: verify the working of method
                        return res;
                    });
                }
    ...
    root.Collection = root.Resource.extend({
        ...
        fetch: function(options, response_timeout) {
            ...
            var that = this;
            var req = that.get("", options, response_timeout);
            return req.then((res) => {
                that._load(res.data);
                return that;
            }).catch((err) => {
        ...
        create: function(params, response_timeout) {
            var that = this;
            var req = this.post("", params, response_timeout).then((response)=>{
                ...
                if (that.fetchOnEntityCreation) {
                    entity.fetch({}, response_timeout).then((res) => {
                        // TODO: verify the working of method
                        return res;
                    })
                }
    ...
    root.SavedSearch = root.Entity.extend({
        ...
        acknowledge: function(response_timeout) {    
            var that = this;
            var req = this.post("acknowledge", {},response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        dispatch: function(options, response_timeout) {
            ...
            var that = this;
            var req = this.post("dispatch", options, response_timeout).then((response) => {
        ...
        update: function(params, response_timeout) {
            ...
            if (!params.search) {
                ...
                var req = this.fetch({}, response_timeout).then((search) => {
                    params.search = search.properties().search;
                    // TODO: verify the working of method
                    update.call(search, params, response_timeout);
                });
    ...
    root.Configurations = root.Collection.extend({
        ...
        create: function(filename, response_timeout) {
            ...
            var that = this;
            var req = this.post("", {__conf: filename}, response_timeout).then((response) => {
    ...
    root.Job = root.Entity.extend({
        ...
        cancel: function (response_timeout) {
            var req = this.post("control", {action: "cancel"}, response_timeout);
        ...
        disablePreview: function(response_timeout) {
            var that = this;
            var req = this.post("control", {action: "disablepreview"}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        enablePreview: function(response_timeout) {
            var that = this;
            var req = this.post("control", {action: "enablepreview"}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        finalize: function(response_timeout) {
            var that = this;
            var req = this.post("control", {action: "finalize"}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        pause: function(response_timeout) {  
            var that = this;
            var req = this.post("control", {action: "pause"}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        setPriority: function(value, response_timeout) {
            var that = this;
            var req = this.post("control", {action: "setpriority", priority: value}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        setTTL: function(value, response_timeout) {
            var that = this;
            var req = this.post("control", {action: "setttl", ttl: value}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        touch: function(response_timeout) {
            var that = this;
            var req = this.post("control", {action: "touch"}, response_timeout).catch((err) => {
                throw [err, that];
            });
        ...
        track: function(options, callbacks) {
            // TODO: update method to support promise
        ...
        unpause: function(response_timeout) {
            var that = this;
            var req = this.post("control", {action: "unpause"}, response_timeout).catch((err)=>{
                throw [err, that];
            });
    ...
    root.Pivot = Class.extend({
        ...
        run: function(args, response_timeout) {
            ...
            this.service.search(this.tstatsSearch || this.pivotSearch, args, response_timeout);
        }
    });
    ...
    root.PivotSpecification = Class.extend({
        ...
        run: function(args, response_timeout) {
            ...
            this.pivot().then((pivot) => {
                // TODO: After Async.augment method changes, modify the parameters
    ...
    root.DataModelObject = Class.extend({
        ...
        createLocalAccelerationJob: function(earliestTime, response_timeout) {
            ...
            this.dataModel.service.search(query, args, response_timeout);
        },
        ...
        startSearch: function(params, querySuffix, response_timeout) {
            ...
            this.dataModel.service.search(query + querySuffix, params, response_timeout);
        },
    ...
    root.DataModel = Service.Entity.extend({
         ...
         */
        update: function(props, response_timeout) {
            if (utils.isNumber(props) && !response_timeout) {
    ...
    root.PaginatedEndpointIterator = Class.extend({
        ...
        next: function(response_timeout) {
            ...
            return this._endpoint(params, response_timeout).then((results) => {
