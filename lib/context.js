    ...
    module.exports = root = Class.extend({
        ...
        getVersion: function (response_timeout) {
            ...
            return this.http.get(
                ...
                var hasVersion = !!(response.data && response.data.generator.version);
                
                if (!hasVersion) {
                    Promise.reject("No version found");
                }
                else{
        ...
        login: function(response_timeout) {
            ...
            return this.http.post(
                ...
                var hasSessionKey = !!(response.data && response.data.sessionKey);
                if (!hasSessionKey) {
                    Promise.reject("No session key available");
                }
                else {
                    that.sessionKey = response.data.sessionKey;
                    that.getVersion(response);
                }
            })
        },
         ...
         * @private
         */
        logout: function(response_timeout) {
            // TODO : check working of response_timeout parameter
