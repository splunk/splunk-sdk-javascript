...
(function() {
    ...
    module.exports = root = Class.extend({
        ...
        init: function(http, params) {
            ...
            this.prefix = this.scheme + "://" + this.host + ":" + this.port;
        ...
        login: function(callback) {
            ...
            return this.http.post(
                this.urlify(url) + "?output_mode=json",
                this._headers(),
                params,
                0,
                wrappedCallback
        ...
        get: function(path, params, callback) {
            // Set the output_mode=json if necessary
            if (params.hasOwnProperty("output_mode")) {
                var outputMode = params["output_mode"];
        ...
        del: function(path, params, callback) {
            ...
            if (params.hasOwnProperty("output_mode")) {
                var outputMode = params["output_mode"];
                if (!utils.startsWith(output_mode, "json")) {
                    output_mode = "json";
                }
            }
            else {
        ...
        post: function(path, params, callback) {
            ...
            var request = function(callback) {
                return that.http.post(
                    that.urlify(path) + "?output_mode=json",
                    that._headers(),
                    params,
                    0,
                    callback
        ...
        request: function(path, method, headers, body, callback) {
            ...
            var request = function(callback) {
                return that.http.request(
                    that.urlify(path) + "?" + Http.encode(params),
                    {
                        method: method,
