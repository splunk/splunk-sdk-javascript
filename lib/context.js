...
(function() {
    ...
    module.exports = root = Class.extend({
        ...
        init: function(http, params) {
            ...
            this.autologin     = params.hasOwnProperty("autologin") ? params.autologin : true;
            ...
            this._makeRequest     = utils.bind(this, this._makeRequest);
        },
        ...
        /*!*/
        _makeRequest: function(task, callback) {
            var that = this;
            var req = null;
                ...
                if (err && err.status === 401 && that._shouldAutoLogin()) {
                    ...
                    that.login(function(err, success) {
                        ...
                        if (err) {
                            ...
                            // If we just failed to login, send it through as well
