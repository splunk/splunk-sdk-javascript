...
(function() {
    ...
    root.PivotSpec = Class.extend({
        ...
        init: function(dataModelObject) {
            ...
            this._comparisonTypes[this.comparisonBoolean] = {EQUALS: "="};
            // TODO: the rest

            // TODO: simplified below
            if (dataModelObject.dataModel().isAccelerated()) {
                this.accelerationNamespace = dataModelObject.dataModel().name;
            }
            else {
                this.accelerationNamespace = null;
            }
            // this.accelerationNamespace = dataModelObject.isAccelerated() ? dataModelObject.name : null;
        },
        ...
        addFilter: function(fieldName, comparisonType, comparison, compareTo) {
            if (this._dataModelObject.hasField(fieldName)) {
                if (comparisonType === this._dataModelObject.fieldByName(fieldName).type) {
                    if (this._comparisonTypes[comparisonType].hasOwnProperty(comparison)) {
                        this.filters.push({
                            fieldName: fieldName,
                            dataModelObject: this._dataModelObject,
                            type: comparisonType
                            // TODO: other methods
                        });
                    }
