...
(function() {
    ...
    root.PivotSpec = Class.extend({
        ...
        pivot: function(callback) {
            ...
            console.log(args.pivot_json);
            if (!utils.isUndefined(this.accelerationNamespace)) {
                args.namespace = this.accelerationNamespace;
            }

            svc.get(Paths.pivot + "/" + encodeURIComponent(this._dataModelObject._dataModel.name), args, function(response) {
                if (response.response.statusCode === 500) {
                    callback(new Error(response.data.messages[0].text), response);
                }
                else {
