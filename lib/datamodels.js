...
(function() {
    ...
    root.PivotSpec = Class.extend({
        ...
        addRangeRowSplit: function(field, label, start, end, step, limit) {
            ...
            if (!utils.isUndefined(end)) {
                ranges.end = end;
            }
            if (!utils.isUndefined(step)) {
                ranges.size = step;
            }
        ...
        pivot: function(callback) {
            ...
            return svc.get(Paths.pivot + "/" + encodeURIComponent(this._dataModelObject._dataModel.name), args, function(err, response) {
                if (err) {
                    ...
                    // TODO: in the end, we want to return a Pivot object, probably create a new class for it.
                    if (response.data.entry && response.data.entry[0]) {
                        ...
                        var pivot = {
                            ...
                            tstatsSearch: content["tstatsSearch"] || null,
                            // TODO: docs
                            run: function(args, callback) {
                                ...
                                if (this.tstatsSearch) {
                                    this.service.search(this.tstatsSearch, args, callback);
                                }
                                else {
                                    this.service.search(this.pivotSearch, args, callback);
                                }
                            }
                        };
                        callback(null, pivot);
