...
(function() {
    ...
    root.DataModelField = Class.extend({
        ...
        /**
         * Returns the owner, a data model object, of this data model field.
         *
    ...
    root.DataModelConstraint = Class.extend({
        ...
        /**
         * Returns the owner, a data model object, of this data model constraint.
         *
    ...
    root.PivotSpec = Class.extend({
        ...
        init: function(dataModelObject) {
            ...
            // comparison type constants, TODO: cleanup; also it may help to move the types to DataModelField
            this.comparisonBoolean = "boolean";
            this.comparisonString = "string";
            this.comparisonNumber = "number";
            this.comparisonIPv4 = "ipv4";
            this.comparisonObjectCount = "objectCount"; // TODO: should be lowercase, but some test says otherwise
            this.comparisonChildCount = "childCount"; // TODO: should be lowercase, but some test says otherwise
            this.comparisonTimestamp = "timestamp";
        ...
        addFilter: function(fieldName, comparisonType, comparison, compareTo) {
            ...
            if (!this._dataModelObject.hasField(fieldName)) { // TODO: use this check in row splits
        ...
        addRowSplit: function(field, label) {
            if (!this._dataModelObject.hasField(field)) {
                throw new Error("Did not find field " + field);
            }
            var f = this._dataModelObject.fieldByName(field);
            if (!utils.contains([this.comparisonNumber, this.comparisonString], f.type)) {
            ...
            if (f.type === this.comparisonNumber) {
                row.display = "all";
            }
        ...
        addRangeRowSplit: function(field, label, start, end, step, limit) {
            ...
            var f = this._dataModelObject.fieldByName(field);
            if (this.comparisonNumber !== f.type) {
        ...
        addBooleanRowSplit: function(field, label, trueDisplayValue, falseDisplayValue) {
            if (!this._dataModelObject.fieldByName(field)) {
                throw new Error("Did not find field " + field);
            }
            var f = this._dataModelObject.fieldByName(field);
            if (this.comparisonBoolean !== f.type) {
        ...
        addTimestampRowSplit: function(field, label, binning) {
            ...
            var f = this._dataModelObject.fieldByName(field);
            if (this.comparisonTimestamp !== f.type) {
        ...
        addCellValue: function(field, label, statsFunction) {
            ...
            var f = this._dataModelObject.fieldByName(field);
            if (utils.contains([this.comparisonString, this.comparisonIPv4], f.type) &&
                !utils.contains([
            ...
            else if (this.comparisonNumber === f.type && 
                !utils.contains([
            ...
            else if (this.comparisonTimestamp === f.type &&
                !utils.contains([
        ...
        toJSON: function() {
            var ret = {
                "dataModel": this._dataModelObject._dataModel.name,
                "baseClass": this._dataModelObject.name,
        ...
        pivot: function(callback) {
            var svc = this._dataModelObject._dataModel._service;
            ...
            return svc.get(Paths.pivot + "/" + encodeURIComponent(this._dataModelObject._dataModel.name), args, function(err, response) {
                if (err) {
    ...
    root.DataModelObject = Class.extend({
        ...
        init: function(props, parentDataModel) {
            ...
            this.lineage = props.lineage.split("."); // TODO: probably need some fallback if lineage is undefined
            this.parent = this.dataModel().objectByName(this.parentName);
    ...
    root.DataModel = Service.Entity.extend({
        ...
        init: function(service, name, namespace, props) {
            ...
            if (props.content[this._ACCELERATION_LABEL]) {
                ...
                if (props.content.acceleration.hasOwnProperty("enabled")) {
                    ...
                    this.acceleration.enabled = !!this._properties.acceleration.enabled;
                }
            }
        ...
        update: function(callback) {
            ...
            var props = {
                acceleration: {
                    enabled: this.acceleration.enabled,
                    earliest_time: this.acceleration.earliestTime,
                    cron_schedule: this.acceleration.cronSchedule
                }
            };
