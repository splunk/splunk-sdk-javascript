...
(function() {
    ...
    root.callbackToObject = function(callback) {
        ...
        return {
            success: callback.success ? callback.success : function() {
                var args = root.toArray(arguments);
                args.unshift(null);
                callback.apply(null, args);
            },
            error: callback.error ? callback.error : function() {
                var errorArgs = root.toArray(arguments);
                errorArgs = errorArgs.length === 1 ? errorArgs[0] : errorArgs;
                var args = [errorArgs, null];
