...
(function() {
    ...
    root.augment = async function(fn, args){
        let response = [];
        response.push(args);
        if(root.isFunction(fn)){
            return fn().then(res =>{
                    return [res, ...response];
                }).catch((err) => {
                    throw [err, ...response];
    ...
    root.whilst = function(condition, body){
    ...
    root.parallelMap = async function (vals, fn) {
        ...
        return await root.parallel(tasks);
    }
    ...
    root.parallelEach = async function (vals, fn) {  
        ...
        try{
            await root.parallelMap(vals, fn);
            // return;
        } catch (err) {
            throw err;
        }
