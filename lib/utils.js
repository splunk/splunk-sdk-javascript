...
(function() {
    ...
    // TODO: replace this with the underscore object equals function. (DVPL-4289)
    /**
     * Compare 2 objects for deep equality.
     *
     * @param {Object} actual An object.
     * @param {Object} expected Another object.
     * @return {Boolean} `true` if the objects are equal, `false` if not.
     */
    root.deepEquals = function (actual, expected) {
        var a = [], b = [];
        root.forEach(actual, function (elem, key, obj) {
            a.push(key);
            a.push(elem);
        });
        root.forEach(expected, function (elem, key, obj) {
            b.push(key);
            b.push(elem);
        });
        ...
        a = a.sort();
        b = b.sort();
        root.forEach(actual, function (elem, i, obj) {
            if (a[i] !== b[i]) {
                areEqual =  false;
            }
        });
