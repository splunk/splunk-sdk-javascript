...
(function() {
    ...
    Script.prototype.runScript = function(args, eventWriter, inputStream, callback) {
        ...
        if (args.length === 1) {
            ...
            if (inputStream.on) {
                inputStream.on("data", function(chunk) {
                    ...
                    if (bigBuff.toString("utf8", bigBuff.length-9, bigBuff.length-1) === "</input>") {
                        ...
                        InputDefinition.parse(bigBuff.toString("utf8", 0, bigBuff.length-1), function(err, inputDef) {
                            if (err) {
                                callback(err, 1);
                            }
                            else {
                                that._inputDefinition = inputDef;
            ...
            // Else, we assume inputStream is a Buffer; TODO: this is jut a temporary hack.
            else {
                Async.chain([
                        function(done) {
