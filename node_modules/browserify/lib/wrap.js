...
Wrap.prototype.addEntry = function (file, opts) {
    ...
    this.append((self.debug ? wrappers.entry_debug : wrappers.entry)
        .replace(/\$__filename/g, function () {
            return JSON.stringify(target).replace(/\\/g, "/").replace(/^[A-Za-z]:\//, "/");
        })
...
function makeTarget (file, root) {
    ...
    if (isPrefixOf(path.join(path.normalize(root), "/"), file)) {
        return path.normalize('/' + file.slice(root.length + 1)).replace(/\\/g, "/").replace(/^[A-Za-z]:\//, "/");
    }
    else if (resolve.isCore(file) || !file.match(/^\//)) {
        return path.normalize(file).replace(/\\/g, "/").replace(/^[A-Za-z]:\//, "/");
    }
    ...
    && file.slice(s.length), /^[^\/]+-browserify/.test(file.slice(s.length))) {
        ...
        return path.normalize(file.match(/(\/node_modules\/.+)/)[1]).replace(/\\/g, "/").replace(/^[A-Za-z]:\//, "/");
    }
    else {
